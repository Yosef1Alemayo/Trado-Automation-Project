...
class MaxBussinesPage:
    ...
    def click_link(self):
        self.driver.find_element(By.ID, self.maxlink).click()
    ...
    def enter_phoneNumber(self, Phone):
        self.driver.find_element(By.TAG_NAME, self.phoneNumber).clear()
        self.driver.find_element(By.ID, self.phoneNumber).send_keys(Phone)
    ...
    def enter_stNumber(self, stNumber):
        self.driver.find_element(By.ID, self.streetNumber).clear()
        self.driver.find_element(By.ID, self.streetNumber).send_keys(stNumber)

    @allure.step("Enter business name as {1}")
    def enter_bussinesName(self, bussinesname):
        self.driver.find_element(By.ID, self.businesName).clear()
        self.driver.find_element(By.ID, self.businesName).send_keys(bussinesname)

    @allure.step("Enter business ID as {1}")
    def enter_bussinesID(self, bussinesID):
        self.driver.find_element(By.ID, self.businesID).clear()
        self.driver.find_element(By.ID, self.businesID).send_keys(bussinesID)

    @allure.step("Enter postal code as {1}")
    def enter_postalCode(self, postalCode):
        self.driver.find_element(By.ID, self.postalCode).clear()
        self.driver.find_element(By.ID, self.postalCode).send_keys(postalCode)

    @allure.step("Click on send details")
    def click_send(self):
        self.driver.find_element(By.XPATH, self.sendButton).is_enabled()
